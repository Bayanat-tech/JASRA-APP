...
export const saveFile = async (
  ...
  const successfulRecords: { org_file_name: string; sr_no: number; aws_file_locn?: string }[] = [];

  try {
    for (const file of files) {
      const org_file_name = (file.org_file_name || "").slice(0, 400);
      const aws_file_locn = (file.aws_file_locn || "").slice(0, 500);

      // Check for duplicate entry by request_number and org_file_name
      ...
      const count = duplicateCheckResult.rows?.[0]?.COUNT ?? duplicateCheckResult.rows?.[0]?.[0] ?? 0;
      if (count > 0) {
        duplicateRecords.push(org_file_name);
        continue;
      }

      // Sanitize extension to max 5 characters for VARCHAR2(5)
      let cleanExtension = (file.extensions || "").toLowerCase().replace(/^\./, "");
      if (cleanExtension.length > 5 || !cleanExtension) {
        const extFromName = org_file_name.split(".").pop()?.toLowerCase() || "";
        cleanExtension = extFromName.slice(0, 5);
      } else {
        cleanExtension = cleanExtension.slice(0, 5);
      }

      const user_file_name = (file.user_file_name || org_file_name || "").slice(0, 75);
      const file_name = (file.file_name || org_file_name || "").slice(0, 180);
      const company_code = (file.company_code || req.user?.company_code || "").slice(0, 7);
      const flow_level = file.flow_level !== undefined && file.flow_level !== null ? Number(file.flow_level) : 0;
      const modules = (file.modules || "").slice(0, 50);
      const updated_by = (file.updated_by || req.user?.loginid || "").slice(0, 50);
      const created_by = (file.created_by || req.user?.loginid || "").slice(0, 20);
      ...
      await oracleDb.query(
        `INSERT INTO UPLOADED_FILES_DLTS (
          ...
          :company_code, :request_number, :file_name, :cleanExtension, :org_file_name, 
        ...
        )`,
        {
          ...
          cleanExtension: cleanExtension || null,
      ...
      // Fetch the generated SR_NO - prefer matching by AWS_FILE_LOCN as it's unique
      let srNoResult: any;
      if (aws_file_locn) {
        srNoResult = await oracleDb.query(
          `SELECT SR_NO 
           FROM UPLOADED_FILES_DLTS 
           WHERE request_number = :request_number AND aws_file_locn = :aws_file_locn`,
          { request_number, aws_file_locn }
        );
      }

      if (!srNoResult?.rows?.[0]) {
        srNoResult = await oracleDb.query(
          `SELECT SR_NO
           FROM (
             SELECT SR_NO
      ...
      const generatedSrNo = srNoResult?.rows?.[0]?.SR_NO ?? srNoResult?.rows?.[0]?.[0];
      if (generatedSrNo !== undefined && generatedSrNo !== null) {
        successfulRecords.push({
          org_file_name,
          sr_no: Number(generatedSrNo),
          aws_file_locn,
        });
      }
    }
