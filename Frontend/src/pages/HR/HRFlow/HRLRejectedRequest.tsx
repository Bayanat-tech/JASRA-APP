...
const MASTER = 'Pg_leave_flow_Rejected';
...
const HRLRejectedRequest: FC<HRLRejectedRequestProps> = () => {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `Rejected_Leave_Requests_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}.xlsx`);
  ...
  const columnDefs = useMemo<ColDef<TLeaveApproval>[]>(
    () => [
      ...
      },
      {
        ...
        filter: false,
        cellRenderer: () => (
          <Chip label="Rejected" size="small" color="error" variant="filled" sx={{ fontWeight: 500 }} />
        )
      },
      {
